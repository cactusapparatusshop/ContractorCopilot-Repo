...
export default async function JobsPage() {
  ...
  return <>
    ...
    <section className="card" style={{ overflow: "hidden" }}>
      ...
      {jobs.length ? <div className="table-wrap"><table className="data-table"><thead><tr><th>Job</th><th>Client</th><th>Address</th><th>Proposal</th><th>Status</th><th>Updated</th><th /></tr></thead><tbody>{jobs.map((job) => {
        ...
        return <tr key={job.id}><td className="strong">{job.title}</td><td><div className="customer-cell"><span className="table-avatar">{viewerInitials(client)}</span><span className="strong">{client}</span></div></td><td>{job.address || "—"}</td><td className="strong">{estimate ? formatMoney(estimate.totalCents, estimate.currency) : "Not drafted"}</td><td><StatusBadge status={status} /></td><td>{job.updatedAt.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</td><td><Link aria-label={`Open ${job.title}`} href={destination} className="icon-button"><ChevronRight size={15} /></Link></td></tr>;
