...
export function ProposalDocument({ portal = false, data = demoProposalDocument }: { portal?: boolean; data?: ProposalDocumentData }) {
  const paperClass = portal ? "portal-paper" : "proposal-paper";
  const customerAddress = data.customer.address?.split("\n").filter(Boolean) ?? [];
  return <article className={paperClass}>
    ...
    <table className="proposal-table"><thead><tr><th>Description</th><th>Quantity</th><th>Amount</th></tr></thead><tbody>{data.lines.map((line) => <tr key={`${line.item}-${line.quantity}`}><td>{line.item}</td><td>{line.quantity}</td><td>{currency(line.amount, 2)}</td></tr>)}</tbody></table>
